...
WECHAT_TOKEN = "young"
WECHAT_APP_ID = "testID"
WECHAT_APP_SECRET = "testSerect"
...
class WechatHandler(RequestHandler):
	...
	def post(self):
		...
		if msg_type == "text":
			...
			resp_data = {
				"xml":{
					"ToUseName": dict_data["xml"]["FromUserName"],
			...
			if dict_data["xml"]["Event"] == "subscribe":
				...
				if "EventKey" in dict_data["xml"]:
					...
					resp_data["xml"]["Content"] = u"终于等到你 %s" % scene_id
				self.write(xmltodict.unparse(resp_data))
...
def main():
	...
	app = tornado.web.Application(
		...
		template_path=os.path.join(os.path.dirname(__fine__), "template")
	)
